...
{
    public class LargeWaterTank
    {
        private readonly double[,,] doseGrid;
        public int Nx { get; set; }
        public int Ny { get; set; }
        public int Nz { get; set; }
        public double[,] xSlice { get; set; }
        public double[,] ySlice { get; set; } 
        public double VoxelSizeCm { get; set; }
        public object Bitmap { get; private set; }

        public LargeWaterTank(int nx, int ny, int nz, double voxelSizeCm)
        {
        ...
        public double[,] GetSlice2D_X(int xIndex)
        {
            double[,] slice = new double[Ny, Nz];

            for (int iy = 0; iy < Ny; iy++)
                for (int iz = 0; iz < Nz; iz++)
                    slice[iy, iz] = doseGrid[iy, xIndex, iz];
                    xSlice = slice;
            return xSlice;
        }

        public void ExportSliceX(int xIndex, string filename, double numPhotons, double photonEnergyMeV)
        {
            var normalizedDoseGrid = NormalizeToIncidentEnergy(numPhotons, photonEnergyMeV);
            using var writer = new StreamWriter(photonEnergyMeV + "_" + filename);
            ...
            {
                double xCoord = iy * VoxelSizeCm;
                writer.Write($"{xCoord:F2};");

                for (int iz = 0; iz < Nz; iz++)
                {
                    double val = normalizedDoseGrid[xIndex, iy, iz];
                    writer.Write($"{val:E4};");
                }
                writer.WriteLine();
            }
            ...
            Console.WriteLine($"Exported Y-slice {xIndex} to CSV: {filename}");
        }
    }
}
